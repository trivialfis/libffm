...
// ffm_int const kCHUNK_SIZE = 1000;
...
ffm_long get_w_size(ffm_model &model) {
  ...
  return (ffm_long) model.n * model.m * k_aligned * 2; // * 2 for gradients
}
...
ffm_model ffm_train_on_disk_cl(string tr_path, string va_path, ffm_parameter param)
{
  ...
  ffm_int auto_stop = param.auto_stop && !va_path.empty();

  ffm_long w_size = get_w_size(model);
  // std::cout << model.m << " " << model.n << " " << model.k
  // 	    << " get_k_aligned: " << get_k_aligned(model.k) << " w size: "
  // 	    << w_size << std::endl;
  ...
  clloop::CLWrapper cl;
  clloop::CLKernel kernel = cl.eval("update_block.cl", "update_block");
